...
PID 删除会释放空闲 VID 和线程链表节点；PID 迁移直接移动原线程节点，并复用
空闲 VID 节点，避免反复分配持久节点引起堆块大小变化。01 保留严格的堆用量
相等断言，同时检查根 PID 成员关系和引用数。子 IPC 容器只初始化私有对象池，
不覆盖全局对象注册项；01 检查注册项在子容器创建和删除前后保持不变，避免
资源快照读取已释放的对象信息。
